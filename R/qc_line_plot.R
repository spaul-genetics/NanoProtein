...
#' @param order_by How the ROIs are ordered. Default is geometric mean 'GeoMean' if target contains more than one elements. Possible value is a background IgG protein or a housekeeping protein
#'
#' @author Subrata Paul
#' @export qc_line_plot
#' @import ggplot2
...
qc_line_plot<-function(nano_obj,
  ...
  if(length(target_features)==1){
    order_by = target_features
  }

  to_order_roi = plot_data[plot_data$Group==order_by,]
  order_roi = as.character(to_order_roi$ROI[order(to_order_roi$Count)])

  plot_data$ROI = factor(plot_data$ROI, levels = order_roi)
  ...
  if(length(target_features)==1){
    col_breaks = c(target_features, shadow)
    col_values = c('red', "#BDBDBD")
  }else{
