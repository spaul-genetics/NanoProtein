...
#' @param method Normalization method. One of hghk, posonly, 'Endogenous', or 'hk'
...
normalize_nano<-function(nano_obj, method = 'bghk', out_name = NA, used_igg, used_hk){
  ...
  if(sum(nano_obj@rows$CodeClass=="Positive")>1){
    ...
    roiSpikein <- exp(colMeans(log(nano_obj@count[which(nano_obj@rows$CodeClass=="Positive"),])))
    scale.factor <- mean(roiSpikein)/roiSpikein
  ...
  nano_obj@count_norm[['Positive']]<- sweep(x = nano_obj@count, MARGIN = 2, STATS = scale.factor, FUN = "*")
  nano_obj@count_norm[['Norm.Scale']] <- data.frame(Positive = scale.factor)

  if(method == 'posonly'){
    return(nano_obj)
  }

  if(method == 'bghk' | method == 'all'){
    if(is.na(out_name)) out_name = 'bghk'
    ...
    housekeeper_to_scale = mean(housekeeper_to_scale)/housekeeper_to_scale
    ...
    nano_obj@count_norm[['Norm.Scale']][[out_name]]<-housekeeper_to_scale
    #return(nano_obj)
  }


  if(method == 'hk' | method == 'all'){
    if(is.na(out_name) | method == 'all') out_name = 'hk'
    housekeeper_to_scale = exp(colMeans(log(nano_obj@count_norm[['Positive']][used_hk,])))
    housekeeper_to_scale = mean(housekeeper_to_scale)/housekeeper_to_scale
    endogenous <- rownames(nano_obj@rows)[nano_obj@rows$CodeClass=='Endogenous']
    #endogenous = setdiff(endogenous, unused_endo)
    nano_obj@count_norm[[out_name]] = sweep(x = nano_obj@count_norm[['Positive']], MARGIN = 2, STATS = housekeeper_to_scale, FUN = '*')
    nano_obj@count_norm[['Norm.Scale']][[out_name]]<-housekeeper_to_scale
    #return(nano_obj)
  }


  if(method == 'Endogenous' | method == 'all'){
    if(is.na(out_name) | method == 'all') out_name = 'Endogenous'
    endogenous <- rownames(nano_obj@rows)[nano_obj@rows$CodeClass=='Endogenous']
    scale_factor = exp(colMeans(log(nano_obj@count_norm[['Positive']][endogenous,])))
    scale_factor = mean(scale_factor)/scale_factor
    nano_obj@count_norm[[out_name]] = sweep(x = nano_obj@count_norm[['Positive']][endogenous,], MARGIN = 2, STATS = scale_factor, FUN = '*')
    nano_obj@count_norm[['Norm.Scale']][[out_name]]<-scale_factor
    #return(nano_obj)
  }
